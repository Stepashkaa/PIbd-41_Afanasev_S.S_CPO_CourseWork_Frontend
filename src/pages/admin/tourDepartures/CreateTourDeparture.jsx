  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        capacityReserved: formData.capacityReserved === "" ? 0 : Number(formData.capacityReserved),
        priceOverride: formData.priceOverride ? Number(formData.priceOverride) : null,
        tourId: Number(formData.tourId),
        flightIds: Array.from(selectedFlightIds),
      };

      await createTourDeparture(payload);
      navigate("/tour-departures");
    } catch (e2) {
      setError(e2?.response?.data?.message || "Ошибка создания вылета тура");
    }
  };
          ...
          <Card.Body>
            ...
            <div className="d-flex gap-3 flex-wrap mt-3">
              ...
              <Form.Group style={{ minWidth: 220 }}>
                ...
                <Form.Control type="number" min="0" name="capacityReserved" value={formData.capacityReserved} onChange={handleChange} />
              </Form.Group>
        ...
        <Card className="mb-3">
          <Card.Body>
            <div className="d-flex align-items-center justify-content-between mb-2">
              <h5 className="mb-0">
                Привязать рейсы <Badge bg="secondary">{selectedCount}</Badge>
              </h5>
            </div>
