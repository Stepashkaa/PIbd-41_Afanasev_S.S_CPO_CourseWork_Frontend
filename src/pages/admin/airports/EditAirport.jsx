...
import { getAirportById, updateAirport } from "../../../services/airportService.js";
...
export default function EditAirport() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [airport, cityList] = await Promise.all([
          getAirportById(id),
          getCitiesAll(),
        ]);
        ...
        setFormData({
          ...
          name: airport.name || "",
          cityId: airport.cityId ? String(airport.cityId) : "",
        });
      } catch (e) {
        setError(e?.response?.data?.message || "Ошибка загрузки аэропорта");
      } finally {
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err?.response?.status === 400 && message) {
        setError(message); // например “Аэропорт с таким IATA-кодом уже существует”
      } else {
        setError("Ошибка обновления аэропорта");
      }
    }
  };
  ...
  return (
    <Container className="mt-3" style={{ maxWidth: 640 }}>
      <h3>Редактировать аэропорт #{id}</h3>
